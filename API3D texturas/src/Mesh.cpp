	...
	if (result) {
		...
		for (pugi::xml_node bufferNode = buffersNode.child("buffer");
			...
			if (materialNode.child("blend")) {
				...
				if (blendModeStr == "alpha") material.setMode(MaterialMode::ALPHA);
				else if (blendModeStr == "add") material.setMode(MaterialMode::ADD);
				else if (blendModeStr == "mul") material.setMode(MaterialMode::MUL);
